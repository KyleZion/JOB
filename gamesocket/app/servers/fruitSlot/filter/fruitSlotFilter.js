...
Filter.prototype.before = function (msg, session, next) {
  var lockAccount = 0;
  if(msg.route == "fruitSlot.fruitSlotHandler.X"){
    var cost = 0;
    async.series({
      lockAccount: function(callback){ //redis修正
        redis.sismember("GS:lockAccount:fruitSlot",session.uid,function(err,res){
          if(res==0){
            redis.sadd("GS:lockAccount:fruitSlot",session.uid);
            lockAccount = 1;
            callback(null,200);
          }
          else{
    ...
    function(err, res)
    {
      ...
      {
        //console.log(res); //OK
        var iFilter_Base = new require(pomelo.app.getBase() + "/app/lib/Filter_Base.js")(bypass,msg,next,"fruitSlotFilter"); //放在最後一行
      }
    });//async.series END
  }
  else if(msg.route == "fruitSlot.fruitSlotHandler.A")
  {
    redis.sismember("GS:lockAccount:fruitSlot",session.uid,function(err,res){
      if(res==0){ 
        var iFilter_Base = new require(pomelo.app.getBase() + "/app/lib/Filter_Base.js")(bypass,msg,next,"fruitSlotFilter"); 
      }
      else{
