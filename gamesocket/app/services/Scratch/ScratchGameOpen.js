...
//          }
        ...
        gameOpenNum = getOpenNum(gameZone,1); 
        for(let nn=0;nn<gameOpenNum.length;nn++){
            var tmpwin = 0;   //總贏分 (玩家贏)
            var c = 0;
            var winbet = new Array();
            num = gameOpenNum[nn];
            tmpwin = gameOpenNum[nn];
        ...
        if(flag==0){
            if (n2positive==false){ 
                console.log( "全部中獎額 每一組都是輸的 找公司輸最少的號碼開");
...
//                  DB_SC = 0;
        ...
        console.log( "累積後彩池bonus444:"+bonus444);
        //======================================================================================
        ...
        redis.hset('GS:Bonus:053', "RedisBonus444", bonus444);
        console.log( "END");
        callback({'ErrorCode':0,'ErrorMessage':'','gameNum':num});

    }
    function getOpenNum(gameZone,type){
        if(type){
            switch(gameZone){
                case 111:
